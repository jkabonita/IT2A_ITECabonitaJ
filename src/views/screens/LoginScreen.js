...
const LoginScreen = ({ navigation }) => {
  ...
  const validate = async () => {
    ...
    if(!inputs.email){
      handleOnError("Please enter your Email Address", "email");
      isValid = false;
    }else if(!inputs.email.match(/\S+@\S+\.\S+/)){
      handleOnError("Please enter a Valid Email Address", "email");
      isValid = false;
    }
    if(!inputs.password){
      handleOnError("Please enter a Password", "password");
      isValid = false;
    }else if(inputs.password.length < 8){
      handleOnError("Minimum Password Lenght is 8", "password");
      isValid = false;
    }
